...
- `embeddings/<model-folder>/` can hold a local sentence-transformers embedding model.
- `rerankers/<model-folder>/` can hold a local cross-encoder reranker.
...
For a stronger fully offline stack, prefer:

- a GGUF instruct model for generation
- a local embedding model for semantic retrieval
- a local reranker for better search precision
